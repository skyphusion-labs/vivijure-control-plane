...
| Abuse intake link on hosted TENANT STUDIO surfaces | DONE | cp#164, verified on the ARTIFACT 2026-07-27 against the live plane at control_plane_version 1.15.0. The converge route (`POST /api/admin/tenants/<id>/abuse-report-url`) was run on the live testbed tenant `rollins-e2e` (id resolved FROM the slug at use time): bindings 19 -> 20, `missing_bindings` and `missing_secrets` both empty, all four tenant secrets intact, and the studio served `host.abuse_report_url: https://studio.vivijure.com/report-abuse` back on `GET /api/modules` with `reader_live: true`, confirmed twice. The value is DERIVED from `CONTROL_PLANE_HOST`, so it is hosted-only by construction: a self-hosted studio never receives the var and renders nothing, which is correct because we cannot act on a self-hoster's content. **Residual, recorded rather than hidden:** the third artifact originally asked for, a human EYEBALLING the link on the rendered panel, needs an owner browser session on the tenant studio; what is proven here is the tenant's own assertion about itself (the field it serves), which is what the panel renders from as its sole signal. **Also found by running it:** the first converge reported a false failure because the readback raced edge propagation; fixed forward with a bounded retry and a 202 outcome, see the cp#164 section of `docs/control-plane.md`. |
