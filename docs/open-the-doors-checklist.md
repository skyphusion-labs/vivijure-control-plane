...
| Abuse intake link on hosted TENANT STUDIO surfaces | PENDING (mechanism landed, artifact unproven) | **Pre-lever, NOT a cf#225 blocker.** cp#164. vivijure-cf v1.10.0 ships the reader (`src/abuse-contact.ts`; projects `host.abuse_report_url`), and the plane now WRITES the var on all three studio write paths: the provision upload, the cp#139 studio upgrade, and `POST /api/admin/tenants/:id/abuse-report-url` for a tenant already live (one door only would leave the estate split, the cp#112 / cp#136 lesson). The URL is DERIVED from `CONTROL_PLANE_HOST` plus `/report-abuse`, so it is hosted-only by construction and carries no hardcoded host: a self-hosted studio never gets the var, and unset renders nothing, which is correct because we cannot act on a self-hoster's content. **What is NOT yet true:** the mechanism has not run against a live tenant, because it ships in an UNRELEASED control plane. This row flips to DONE when a live tenant `GET /api/modules` returns `host.abuse_report_url` and the link is visible on that panel; the route refuses to call that green itself (409 on `reader_live: false`), so the evidence cannot be a pipeline opinion. |
