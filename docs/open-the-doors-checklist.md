...
| Abuse intake link on hosted TENANT STUDIO surfaces | DONE | cp#164, verified on the ARTIFACT 2026-07-27 against the live plane at control_plane_version 1.15.0. **All three artifacts, no residual.** (1) The converge route (`POST /api/admin/tenants/<id>/abuse-report-url`) run on the live testbed tenant `rollins-e2e` (id resolved FROM the slug at use time) returned `reader_live: true`, bindings 19 -> 20, `missing_bindings` and `missing_secrets` both empty, all four tenant secrets intact. (2) The tenant served `host.abuse_report_url: https://studio.vivijure.com/report-abuse` back on its own `GET /api/modules` (`served_url_after`), which is the sole signal `public/abuse-link.js` renders from; confirmed twice a minute apart. (3) The rendered panel was EYEBALLED by the account owner (Conrad) on the testbed studio, 2026-07-27 ~00:35Z: the "Report abuse" link is visible and targets `https://studio.vivijure.com/report-abuse`; screenshot in the lead session. That third one is owner-side by construction and not a gap in the mechanism: the panel is `AUTH_MODE=token` with a dispatcher-injected owner credential, so nobody but the owner has a clean door to it. The value is DERIVED from `CONTROL_PLANE_HOST`, so hosted-only is structural: a self-hosted studio never receives the var and renders nothing, which is correct because we cannot act on a self-hoster's content. **Found by running it:** the first converge reported a false failure because the readback raced edge propagation; fixed forward with a bounded retry and a 202 outcome (see the cp#164 section of `docs/control-plane.md`). |
