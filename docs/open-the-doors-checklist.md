...
| NCMEC ESP registration + 2258A(a)(1)(B)(i) point of contact | PENDING | **BLOCKING.** Human step, cannot be delegated to code. Register via `espteam@ncmec.org`; designate a named individual point of contact. Owner: Conrad. |
| Segregated preservation path exists (location, access control, 1-year clock) | PENDING | **BLOCKING.** 2258A(h)(1) is **1 year** from CyberTipline submission (Pub. L. 118-59; NOT the repealed 90 days, which is 2703(f) on a governmental request). Owner: Conrad + Infra. |
| Teardown interlock on a preservation hold | PENDING | **BLOCKING.** No code prevents irreversible teardown of a tenant under an open hold; only operator memory does. Owner: control-plane owner. |
| Named authorized responder list (who may look, under 2258B(c)) | PENDING | **BLOCKING.** Owner: Conrad. |
| Suspend lever watched fire end to end, with positive control | PENDING | Non-blocking but soon. The lever is built and verified in code (`src/index.ts`, `src/routing.ts`); nobody has watched it fire against a real tenant studio. Owner: control-plane owner. |
