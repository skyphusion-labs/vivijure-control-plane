...
| Item | Status | Evidence |
|---|---|---|
| CSAM bright-line: report-driven enforcement machinery exists | **PARTIAL** (docs DONE, 5 blocking gaps open; see the rows below) | Conrad's ruling: NO proactive scanning -- the privacy policy deliberately does not monitor tenant content; the bright-line's "one exception to the no-surveillance ethos" is surveillance-ON-CAUSE, triggered only by a credible report. What must exist before signups (vivijure-cf#225): credible-report intake path, bounded investigation procedure (the one sanctioned privacy-policy deviation), evidence preserve + tenant freeze (haltable teardown), NCMEC CyberTipline + LE runbook. Refusal already lives monitoring-free in model-side LLM refusals, the curated demo path, and the AUP gate. **Signups do not open before cf#225 ships.** The written machinery landed 2026-07-25 (`ABUSE-RESPONSE-RUNBOOK.md` + `REPORT-ABUSE.md`); what remains is five OPERATIONAL gaps, each its own row below. Documents alone do not open the doors. |
...
| Abuse/support intake path: **`abuse@skyphusion.org` proven deliverable + monitored** | PENDING | **BLOCKING.** The address is already published in the IN-FORCE hub AUP and hosted AUP 1.0.0, and has never been proven to receive mail. Owner: Strummer (delivery), Conrad (monitoring commitment). cp#115. An unread abuse mailbox is how actual knowledge becomes a 2258A(e) failure to report. |
| NCMEC ESP registration + 2258A(a)(1)(B)(i) point of contact | PENDING | **BLOCKING.** Human step, cannot be delegated to code. Register via `espteam@ncmec.org`; designate a named individual point of contact. Owner: Conrad. cp#116. |
| Segregated preservation path exists (location, access control, 1-year clock) | PENDING | **BLOCKING.** 2258A(h)(1) is **1 year** from CyberTipline submission (Pub. L. 118-59; NOT the repealed 90 days, which is 2703(f) on a governmental request). Owner: Conrad + Infra. cp#117. |
| Teardown interlock on a preservation hold | PENDING | **BLOCKING.** No code prevents irreversible teardown of a tenant under an open hold; only operator memory does. Owner: control-plane owner. cp#118. |
| Named authorized responder list (who may look, under 2258B(c)) | PENDING | **BLOCKING.** Owner: Conrad. cp#119. |
| Suspend lever watched fire end to end, with positive control | PENDING | Non-blocking but soon. The lever is built and verified in code (`src/index.ts`, `src/routing.ts`); nobody has watched it fire against a real tenant studio. Owner: control-plane owner. cp#120. |
