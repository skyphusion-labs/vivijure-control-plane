...
| `1.2.0` | (not yet served) | **NOT YET SERVED** (force status is Conrad to state; cannot be served before vivijure-cf#751 is ruled, see below) | **cp#514: 1.1.0 plus a section 3.2 that flows down the model-provider conduct terms.** On the hosted studio, Workers AI FLUX (`@cf/black-forest-labs/*`) and `google/nano-banana-pro` run on OUR Cloudflare account, so Cloudflare binds Skyphusion Labs to BFL's Terms of Service (1.3(n): no using Output "to train, distill or fine-tune any other AI models"; 1.2(c) consent for real, identifiable individuals; 1.3(m), 1.3(p); Usage Policy) and to the Gemini API Additional Terms (18+; no service directed at or likely to be accessed by under-18s; no competing models). Tenants are not parties to those terms, so the AUP is the only instrument that carries them. MINOR: it changes what a person is agreeing to. Also makes the age floor explicit at 18 (1.1.0 says only adult / old enough to contract). NEW VERSION rather than an in-place edit of the unserved 1.1.0 because SHA256SUMS is append-only. **Cannot be served before vivijure-cf#751 is ruled:** the hosted `cast.image` default is CF FLUX and its purpose is a LoRA training set, so serving 3.2 while the product's default button does the thing 3.2 forbids would make the gate contradict the product. Counsel item T1-15. |
