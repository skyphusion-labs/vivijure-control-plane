...
describe("every catalog endpointKey has a transport in the plan (cp#396)", () => {
  ...
  it("EXACTLY ONE TRANSPORT per plan capability, never both and never neither", () => {
    ...
    for (const capability of PROVISION_PLAN) {
      ...
      if (capability.backing === "runpod") {
        ...
        expect(asAny.bindingName, capability.key).toBeUndefined();
        expect(asAny.serviceIdVar, capability.key).toBeUndefined();
      } else {
        expect(typeof asAny.bindingName, capability.key).toBe("string");
        expect(typeof asAny.serviceIdVar, capability.key).toBe("string");
        expect(typeof asAny.doorTokenBinding, capability.key).toBe("string");
        expect(typeof asAny.doorTokenVar, capability.key).toBe("string");
  ...
  it("the vpc binding names are the ones vivijure-cf ACTUALLY declares (cf#480)", () => {
    ...
    // declarations at v1.28.0, the pinned STUDIO_RELEASE.
    const byKey = Object.fromEntries(vpcBackedPlan().map((c) => [c.key, c]));
    expect(byKey.upscale.bindingName).toBe("FINISH_UPSCALE_VPC");
    expect(byKey.upscale.doorTokenBinding).toBe("FINISH_DOOR_TOKEN");
    expect(byKey["audio-upscale"].bindingName).toBe("SPEECH_UPSCALE_VPC");
    expect(byKey["audio-upscale"].doorTokenBinding).toBe("SPEECH_DOOR_TOKEN");
  });
});
